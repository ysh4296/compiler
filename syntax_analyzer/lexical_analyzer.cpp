...
#pragma region Initialization
    ...
    */
    ...
    int token_index = 1;//indicate which position of current token in current line
    char input;         //input character for current index
    vector<ss> answer;  //output string for the result of parsing
    string str = "";    //temp variable for literal string
...
#pragma endregion
    ...
    {
        answer.clear();
        answer.push_back({ "error", "lexical_error : there can't be token named '" + str + "' in " + to_string(token_index / 2) + "th token of line number " + to_string(line_num) });
    }
    ...
    for (int i = 0; i < (int)answer.size(); i++) //after end of parsing, print all the result on console screen
    {
        ...
        if (strcmp(answer[i].first.c_str(), "newLine") == 0)
        {
            writeFile.write("\n", 1);
            continue;
        }
